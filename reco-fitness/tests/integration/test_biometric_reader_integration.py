...
class TestGetRecentReturnsMostRecent:
    def test_returns_latest_among_three_entries(self, db_session):
        ...
        _insert_biometric(
            ...
            measured_at=datetime(2026, 4, 15, 8, 0, tzinfo=timezone.utc),
        )
        _insert_biometric(
            db_session,
            user_id=user_id,
            ...
            measured_at=datetime(2026, 4, 20, 8, 0, tzinfo=timezone.utc),
        )
    ...
    def test_ignores_other_users(self, db_session):
        _insert_biometric(
            db_session, user_id=1, measured_at=datetime(2026, 5, 1, tzinfo=timezone.utc)
        )
        _insert_biometric(
            db_session, user_id=2, measured_at=datetime(2026, 5, 2, tzinfo=timezone.utc)
        )
